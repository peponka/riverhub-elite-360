...
// Esta parte falló antes o quiero rehacerla bien.
code = code.replace(
    /app\.post\('\/api\/ai\/predict-maintenance'[\s\S]*?const userToken = req\.headers\.authorization.*?;/,
    pp.post('/api/ai/predict-maintenance', aiLimiter, authenticateUser, async (req, res) => {
    ...
    try {
        ...
        const { data: profile } = await sb.from('profiles').select('company_id').eq('id', req.user.id).single();
...
code = code.replace(/\?company_id=eq\.\$\{companyId\}/g, '?company_id=eq.');


// 4. Fix supabaseServer en notificaciones broadcast
code = code.replace(
    /const { data: profile } = await supabaseServer\.from\('user_profiles'\)\.select\('role'\)\.eq\('user_id', req\.user\.id\)\.single\(\);/g,
    "const supabaseServer = req.app.locals.supabase;\n        const { data: profile } = await supabaseServer.from('user_profiles').select('role').eq('user_id', req.user.id).single();"
);
