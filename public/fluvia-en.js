...
function renderTracking(filter){
    ...
    if(historyItems.length>0){
        historyItems.forEach(function(v){
            ...
            historyList.innerHTML+='<div class="list-item"><div><h4>'+(v.vessel_name||'--')+' ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ '+(v.destination_port||'--')+'</h4><p>'+(v.origin_port||'')+' | '+(v.cargo_tonss||0)+' tons | '+t+'</p></div><span class="badge" style="color:'+stColor+'">'+stLabel+'</span></div>';
        });
    }
}
