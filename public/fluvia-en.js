...
const EN_DICT = {
    ...
    'TIMONEL': 'HELMSMAN',
    'MAQUINISTA': 'ENGINEER',
    'Vibration detected at 1200 RPM': 'Vibration detected at 1200 RPM',
    'Echosounder calibration - erratic reading': 'Echosounder calibration - erratic reading',
    'Quarterly rudder hydraulic system inspection': 'Quarterly rudder hydraulic system inspection',
    ...
    'Sistema de navigation actualizado y calibrado. Todo nominal.': 'Navigation system updated and calibrated. All nominal.',
    'Automatic System': 'Automatic System',
    'Condiciones normales de navigation': 'Normal navigation conditions',
    'NO ALERTS': 'NO ALERTS',
    'Análisis de condiciones de navigation': 'Navigation conditions analysis',
    'injector': 'injector',
    'CAT 15W-40 Engine Oil': 'CAT 15W-40 Engine Oil',
    'CAT 1R-0716 Oil Filter': 'CAT 1R-0716 Oil Filter',
    '32mm Mooring Line': '32mm Mooring Line',
    'Red Antifouling Paint': 'Red Antifouling Paint',
    'SOLAS Life Jacket': 'SOLAS Life Jacket',
    'Emergency Flares': 'Emergency Flares',
    'Mobilgrease Marine Grease': 'Mobilgrease Marine Grease',
    '7018 Welding Electrode': '7018 Welding Electrode',
    '1/2" Hydraulic Hose': '1/2" Hydraulic Hose',
    'CAT Fuel Filter': 'CAT Fuel Filter',
    'Cordage': 'Cordage',
    ...
    'Navigation': 'Navigation',
    'Navigation Km': 'Navigation Km',
    'NAVIGATION PERMIT': 'NAVIGATION PERMIT',
    'EXPIRED': 'EXPIRED',
    ...
    'Guía completa: Documentación requerida por PREFECTURA para navigation en convoy': 'Complete guide: Documentation required by Coast Guard for convoy navigation',
    'Plataforma de gestión para navigation fluvial': 'River navigation management platform',
    'Resource Center': 'Resource Center',
    'Navigation Fluvial, Logística e Hidrovía': 'River Navigation, Logistics and Waterway',
    'Gestión Inteligente de Flotas Fluviales': 'Intelligent River Fleet Management',
    'Plataforma de gestión inteligente de flotas fluviales para la Paraguay-Parana Waterway.': 'Intelligent river fleet management platform for the Paraguay-Parana Waterway.'
}
...
function showApp(user){
    ...
    sb.from('user_profiles').select('company_id, role, full_name').eq('user_id',user.id).single().then(function(r){
        ...
        if(r.data){
            ...
            console.log('ROLEE:', currentUserRole, 'COMPANY:', currentCompanyId);
...
var modalForms={
    ...
    incidente:{title:'Report Incident',fields:[{id:'inc-title',label:'TITLE',type:'text',placeholder:'Brief incident description'},{id:'inc-vessel',label:'VESSEL',type:'text',placeholder:'Vessel afectada'},{id:'inc-severity',label:'SEVERITY',type:'select',options:['Critical','High','Medium','Low']},{id:'inc-type',label:'TYPE',type:'select',options:['Collision','Grounding','Spill','Mechanical failure','Fire','Medical','Other']},{id:'inc-desc',label:'DESCRIPTION DETALLADA',type:'textarea',placeholder:'What happened, where, when, actions taken...'}]}
};
...
document.getElementById('modal-submit').addEventListener('click',async function(){
    ...
    try{
        ...
        else if(t==='viaje'&&d['viaje-vessel']){await sb.from('voyages').insert({vessel_name:d['viaje-vessel'],origin_port:d['viaje-origin'],destination_port:d['viaje-dest'],cargo_tonss:parseInt(d['viaje-cargo'])||0,departure_date:d['viaje-date']||null,status:'pending',company_id:cid});loadTrips();}
...
function openNewCompanyModal(){
    ...
    document.getElementById('modal-body').innerHTML='<label>NAME DE COMPANY</label><input type="text" id="new-company-name" placeholder="Ex: Guarani Shipping"><label>PLAN</label><select id="new-company-plan" style="width:100%;padding:12px;border-radius:10px;border:0.5px solid var(--separator);font-family:Inter,sans-serif;font-size:14px"><option value="barcaza">Per Barge ($149/mes)</option><option value="combo" selected>Fleet Combo ($899/mes)</option><option value="enterprise">Enterprise ($1,499/mes)</option><option value="ilimitado">Unlimited ($2,499/mes)</option></select><label>MAX VESSELS</label><input type="number" id="new-company-max" placeholder="10" value="10">';
    document.getElementById('modal-overlay').classList.add('open');
