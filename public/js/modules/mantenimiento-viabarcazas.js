/*
 * MANTENIMIENTO KANBAN MODULE (VIABARCAZAS STYLED)
 */

const MaintenanceModuleViaBarcazas = (() => {
    ...
    const init = () => {
        void("🛠️ Mantenimiento (viabarcazasFleet) Iniciando...");
        loadData();
        initCharts();
    };
    ...
    const openCreateModal = () => {
        const existing = document.getElementById('modal-mnt-viabarcazas');
        if (existing) existing.remove();

        const opts = state.vessels.map(v => `<option value="${v.id}">${v.name}</option>`).join('');

        const modalHTML = `
        <div id="modal-mnt-viabarcazas" class="modal-overlay-viabarcazas">
            <div class="modal-content-viabarcazas">
                ...
                <div class="form-group-viabarcazas">
                    ...
                    <select id="mnt-vessel" class="input-viabarcazas">${opts}</select>
                </div>
                
                <div class="form-group-viabarcazas">
                    <label>Nivel de Prioridad</label>
                    <select id="mnt-prio" class="input-viabarcazas">
                ...
                <div class="form-group-viabarcazas">
                    ...
                    <textarea id="mnt-desc" class="input-viabarcazas" rows="3" placeholder="Detalle técnico..."></textarea>
                </div>

                <div class="modal-footer-viabarcazas">
                    <button class="btn-outline" onclick="document.getElementById('modal-mnt-viabarcazas').remove()">Cancelar</button>
                    <button class="btn-primary" onclick="MaintenanceModuleViaBarcazas.submitTask()">Generar Orden</button>
                </div>
            </div>
        </div>
        `;
    ...
    const submitTask = () => {
        ...
        document.getElementById('modal-mnt-viabarcazas').remove();
...
window.MaintenanceModuleViaBarcazas = MaintenanceModuleViaBarcazas;
