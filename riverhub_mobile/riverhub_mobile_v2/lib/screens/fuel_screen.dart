...
class _FuelScreenState extends State<FuelScreen> {
  ...
  Future<void> _fetchData() async {
    ...
    try {
      // Fetch all vessels
      final vesselRes = await Supabase.instance.client
          .from('vessels')
      ...
      final logsRes = await Supabase.instance.client
          .from('fuel_logs')
          .select(
            'id, vessel_id, vessel_name, quantity, log_type, location, logged_at',
          )
  ...
  String _vesselNameFromLog(Map<String, dynamic> log) {
    ...
    if (log['vessel_name'] != null &&
        log['vessel_name'].toString().isNotEmpty) {
      return log['vessel_name'];
    }
    final vesselId = log['vessel_id'];
    if (vesselId != null) {
      final match = _vessels.where(
        (v) => v['id'].toString() == vesselId.toString(),
      );
  ...
  void _addFuelLog() {
    ...
    Map<String, dynamic>? selectedVessel = _vessels.isNotEmpty
        ? _vessels.first
        : null;
          ...
          child: Container(
            ...
            child: ListView(
              children: [
                ...
                // Vessel selector
                Text(
                  LocaleService.t('fuel_select_vessel'),
                  style: GoogleFonts.inter(
                    fontSize: 10,
                    fontWeight: FontWeight.w700,
                    color: AppColors.textSecondary,
                                ...
                                itemBuilder: (_, i) {
                                  ...
                                  return GestureDetector(
                                    onTap: () {
                                      ...
                                      setModalState(() => selectedVessel = v);
                                    },
                                    child: Container(
                ...
                Text(
                  LocaleService.t('fuel_type_label'),
                  style: GoogleFonts.inter(
                    fontSize: 10,
                    fontWeight: FontWeight.w700,
                    color: AppColors.textSecondary,
                ...
                Row(
                  children: [
                    _typeOption(
                      LocaleService.t('fuel_load'),
                      'CARGA',
                      logType,
                      (v) => setModalState(() => logType = v),
                    ),
                    const SizedBox(width: 8),
                    _typeOption(
                      LocaleService.t('fuel_consumption'),
                      'CONSUMO',
                      logType,
                      (v) => setModalState(() => logType = v),
                    ),
                ...
                GestureDetector(
                  onTap: () async {
                    if (amountCtrl.text.isEmpty || selectedVessel == null)
                      return;
                    Navigator.pop(ctx);
                    final amount = double.tryParse(amountCtrl.text) ?? 0;
                    try {
                      if (!selectedVessel!['id'].toString().startsWith(
                        'demo',
                      )) {
                  ...
                  child: Container(
                    ...
                    child: Center(
                      child: Text(
                        LocaleService.t('fuel_confirm_register'),
                        style: GoogleFonts.inter(
                          fontSize: 14,
                          fontWeight: FontWeight.w600,
                          color: AppColors.backgroundPrimary,
      ...
      child: GestureDetector(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(
              color: selected ? AppColors.textPrimary : AppColors.separator,
              width: 0.5,
            ),
                    ...
                    Container(
                      ...
                      child: Column(
                        children: [
                          ...
                          const SizedBox(height: 12),
                          Text(
                            LocaleService.t('fuel_no_records'),
                            style: GoogleFonts.newsreader(
                              fontSize: 18,
                              color: AppColors.textSecondary,
                            ),
  ...
  Widget _fuelCard(Map<String, dynamic> log) {
    ...
    final String logType = (log['log_type'] ?? 'CARGA')
        .toString()
        .toUpperCase();
    final DateTime date =
        DateTime.tryParse(log['logged_at'] ?? '') ?? DateTime.now();
